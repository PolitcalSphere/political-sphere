...
const GameBoard = ({ gameId, proposals, onProposalSubmit, onVote }) => {
  ...
  const handleVote = (proposalId, choice) => {
    if (onVote) {
      onVote({ type: 'vote', payload: { proposalId, playerId: 'current', choice } });
  ...
  return (
    <>
      ...
      <main
        id="main-content"
        tabIndex={-1}
        className={`game-board ${highContrast ? 'high-contrast' : ''} ${largeText ? 'large-text' : ''} ${reducedMotion ? 'reduced-motion' : ''}`}
      ...
      <header>
        ...
        <nav aria-label="Game navigation" tabIndex={-1}>
          <button type="button" onClick={() => focusNextElement()}>Next Proposal</button>
          <button type="button" onClick={() => focusPreviousElement()}>Previous Proposal</button>
        </nav>
      </header>

      <section aria-live="polite" aria-atomic="true" className="sr-only" data-testid="announcements">
