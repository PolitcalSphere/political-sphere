...
export default {
	...
	devServer: {
		port: 3001,
		hot: true,
		static: { directory: path.resolve(root, 'apps', 'remote', 'dist'), watch: { ignored: /node_modules/ } },
		open: false,
		client: { logging: 'warn', overlay: false, progress: false },
		devMiddleware: { stats: 'errors-only' }
	},
	plugins: [
		new HtmlWebpackPlugin({ template: path.resolve(root, 'apps', 'remote', 'public', 'index.html') }),
