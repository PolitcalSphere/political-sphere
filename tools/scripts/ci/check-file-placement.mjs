...
const rules = {
  ...
  root: [
    ...
    '.github/', '.vscode/', '.lefthook/',
    'ai-controls.json', 'ai-metrics.json', 'package-lock.json',
    'TODO-STEPS.md', 'TODO.md'
  ],
...
function checkFilePlacement(filePath) {
  ...
  if (parts.length === 2 && parts[1] === '') { // directory
...
function main() {
  ...
  if (errors.length > 0) {
    ...
    console.error('\nSee governance rules in .github/copilot-instructions.md for placement guidelines.');
    process.exit(1);
  } else {
