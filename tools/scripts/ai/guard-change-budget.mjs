...
console.log(`Files changed: ${totalFilesChanged}, Lines changed (added+deleted): ${totalChangedLines}`);
...
function detectNewDeps(baseRef) {
  try {
    ...
    const depsBase = Object.assign({}, baseJson.dependencies || {}, baseJson.devDependencies || {});
    const depsHead = Object.assign({}, headJson.dependencies || {}, headJson.devDependencies || {});
    const newDeps = [];
...
function checkAuditArtefacts() {
  ...
  const evidencePatterns = [/test-results/i, /screenshots?/i, /test-logs/i, /playwright-report/i, /cypress/i];
  for (const f of changedFiles) {
    for (const p of artPatterns) if (p.test(f)) return { artefactFound: true };
    for (const p of evidencePatterns) if (p.test(f)) return { artefactFound: true, evidenceFound: true };
  }
  // look in repo for common artefact files
  const candidates = ['sbom.json', 'provenance.json', 'artifacts/sbom.json', 'test-results', 'screenshots'];
...
if (mode === 'safe') {
  ...
  if (totalChangedLines > MAX_LINES) {
    fail(`Safe mode budget exceeded: ${totalChangedLines} lines changed (limit ${MAX_LINES}). Reduce changes or switch to Audit mode with justification.`);
  }
  if (totalFilesChanged > MAX_FILES) {
    fail(`Safe mode file-change budget exceeded: ${totalFilesChanged} files changed (limit ${MAX_FILES}). Split change into smaller PRs.`);
  }
  ...
  if (newDeps.length > 0) {
    ...
    if (!hasADR) {
      fail(`Safe mode forbids adding new runtime/build dependencies without an ADR. Detected new deps: ${newDeps.join(', ')}. Attach an ADR or remove the dependency.`);
    }
  }
  ...
  const MAX_FILES = 6;
  if (totalChangedLines > MAX_LINES) {
    fail(`Fast-Secure mode budget exceeded: ${totalChangedLines} lines changed (limit ${MAX_LINES}).`);
  }
  if (totalFilesChanged > MAX_FILES) {
    fail(`Fast-Secure mode file-change budget exceeded: ${totalFilesChanged} files changed (limit ${MAX_FILES}).`);
  }
  if (!checkTodoDeferral()) {
    fail('Fast-Secure mode requires a deferral entry in docs/TODO.md with an owner and due date (YYYY-MM-DD). Please add a TODO deferral.' );
  }
  ...
  if (!artefacts.artefactFound) {
    fail('Audit mode requires SBOM/provenance artefacts (e.g., sbom.json or provenance.json) be included in the change set or present in the repo.');
  }
  // test evidence is recommended — best-effort check
  if (!artefacts.evidenceFound) {
    console.warn('Audit advisory: no explicit test evidence (screenshots/logs) detected in the changed files. CI may still require additional evidence.');
  }
  pass('Audit checks passed (artefacts present)');
} else if (mode === 'r&d' || mode === 'rd' || mode === 'r_d') {
  console.log('R&D mode: advisory only. Ensure PR metadata marks the change as experimental and plan a Safe re-run before merging to protected branches.');
  // Write an advisory file to help reviewers (non-blocking)
  try {
    fs.writeFileSync('.ai-experimental', `experimental: true\nmode: R&D\ndate: ${new Date().toISOString()}\n`);
