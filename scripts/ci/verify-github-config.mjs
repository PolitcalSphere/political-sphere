...
if (fileExists(renovateJson) && fileExists(dependabotYaml)) {
  console.error('ERROR: Both renovate.json and dependabot.yml exist. Keep only one dependency automation config.');
  exitCode = 2;
} else if (!fileExists(renovateJson) && !fileExists(dependabotYaml)) {
  console.error('ERROR: No dependency automation config found. Add either renovate.json or .github/dependabot.yml.');
  exitCode = 2;
} else {
...
if (fileExists(workflowsDir)) {
  ...
  function checkFileName(fileName) {
    ...
    if (!kebabRe.test(fileName)) {
      console.warn(`WARN: Workflow file '${fileName}' does not match kebab-case starting with a lowercase verb token.`);
      ...
      for (const f of nested) {
        ...
        if (/\.ya?ml$/.test(f)) {
          try {
            ...
            execSync('python3', ['-c', 'import sys,yaml; yaml.safe_load(open(sys.argv[1]));', fullPath], { stdio: 'ignore' });
          } catch (e) {
            console.warn(`WARN: YAML parse failed for ${fullPath} (python3/yaml not available or syntax error)`);
            exitCode = Math.max(exitCode, 1);
          }
        }
      }
...
if (fileExists(actionsDir)) {
  ...
  for (const cat of actionCats) {
    ...
    if (fileExists(actionYml)) {
      try {
        execSync('python3', ['-c', 'import sys,yaml; yaml.safe_load(open(sys.argv[1]));', actionYml], { stdio: 'ignore' });
      } catch (e) {
        console.warn(`WARN: YAML parse failed for ${actionYml} (python3/yaml not available or syntax error)`);
        exitCode = Math.max(exitCode, 1);
      }
    }
    ...
    if (fileExists(readme)) {
      ...
      if (!/CHANGELOG/i.test(content)) {
        console.warn(`WARN: README.md for action '${cat.name}' does not mention CHANGELOG or release notes.`);
        exitCode = Math.max(exitCode, 1);
      }
    }
  }
